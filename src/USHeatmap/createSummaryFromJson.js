...
const createSummaryFromJson = (confirmedJson,status) => {
    ...
    for (let i = 0; i < confirmedJson.length; i++) {
        ...
        if (data_point.Status == status) {
            if (data_point.Province in summary) {
                summary[data_point.Province] = summary[data_point.Province] + data_point['Cases'];
            } else {
                summary[data_point.Province] = data_point['Cases'];
...
export default createSummaryFromJson;
