...
const draw = function (geojson, covidjson) {
    ...
    /* const clicktip=function(d){
        console.log('this is ',d3.select(this).select('text'));
        d3.select(this).select('text').style("fill", "black");
        let stateName=String(d.properties.google_name.split("(")[0].trim());
        let notexist=true;
            let newdata=[];
            
            for(let i=0;i<newMap.statesClicked.length;i++){
                let point=newMap.statesClicked[i];
                if(point !== stateName){
                    newdata.push(point);
                }
                else{
            ...
            newMap.statesClicked=newdata;
            newMap.dispatch.call("selected",{},newMap.statesClicked);
